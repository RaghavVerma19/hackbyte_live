...
import { ArrowRight, LockKeyhole, Mail, UserRound, Video } from "lucide-react";
import { ThemeToggle } from "@/components/theme-toggle";
import { AuthRole, fetchJson, saveSession } from "@/lib/auth";

export default function RegisterPage() {
  const router = useRouter();
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [role, setRole] = useState<AuthRole>("interviewer");
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    try {
      const result = await fetchJson<{ token: string; user: any }>("/api/auth/register", {
        method: "POST",
      ...
      <section className="meet-shell flex min-h-[calc(100vh-92px)] items-center justify-center py-10">
        <div className="w-full max-w-md rounded-[2rem] border border-line bg-panel p-8 shadow-[0_24px_72px_rgba(15,23,42,0.12)]">
          ...
          <p className="mt-3 text-sm leading-7 text-muted">
            Register as a candidate or interviewer and continue with JWT-based auth.
          </p>
          ...
          <form onSubmit={onSubmit} className="mt-8 space-y-4">
            ...
            <FieldShell icon={<UserRound className="h-4 w-4 text-muted" />}>
              <select
                value={role}
